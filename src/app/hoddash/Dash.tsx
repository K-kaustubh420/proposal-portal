...
interface Proposal {
    ...
    clarificationMessage?: string; // Added for clarification handling
  }
...
const DashboardContent: React.FC<{
    ...
    closePopup: () => void;
    handleUpdateStatus: (proposal: Proposal, newStatus: string, message?: string) => Promise<void>;
  }> = ({
    eventProposals,
    loading,
    selectedProposal,
    ...
    const [clarificationResponse, setClarificationResponse] = useState(''); // Added for clarification
    ...
    useEffect(() => {
      if (selectedProposal) {
        ...
        setRejectionInput('');
        setReviewInput('');
        setClarificationResponse(''); // Reset clarification input
      }
    }, [selectedProposal]);
    ...
    const cancelRejectReview = () => {
      ...
      setRejectionInput('');
      setReviewInput('');
      setClarificationResponse(''); // Reset clarification
    };
  
    const handleResubmit = () => {
      handleUpdateStatus(selectedProposal!, 'ApprovedByHOD', clarificationResponse).then(() => {
        setClarificationResponse('');
      });
    };
  
    const handleSendBack = () => {
      handleUpdateStatus(selectedProposal!, 'Review', clarificationResponse).then(() => {
        setClarificationResponse('');
      });
    };
...
export default function EventPortal() {
        ...
        try {
          ...
          const proposalsList = proposalSnapshot.docs.map(doc => {
            ...
            return {
              ...
              clarificationMessage: data.clarificationMessage || '', // Added
    ...
    const closePopup = useCallback(() =>
        ...
        const handleUpdateStatus = useCallback(async (proposal: Proposal, newStatus: string, message: string = '') => {
            if (!proposal) return;
            ...
            try {
              ...
              if (newStatus === 'Approved') {
                updatedStatus = 'ApprovedByHOD'; // Transform 'Approved' to 'ApprovedByHOD'
              }
          
              const updateData: any = { proposalStatus: updatedStatus };
              if (newStatus === 'Rejected') {
                updateData.rejectionMessage = message;
              } else if (newStatus === 'Review') {
                updateData.reviewMessage = message;
              } else if (newStatus === 'ApprovedByHOD' && proposal.status === 'AwaitingHODClarification') {
                updateData.clarificationMessage = message;
              }
          
              await updateDoc(proposalRef, updateData);
              console.log(`Proposal ${proposal.id} status updated to ${updatedStatus}`);
          
              // Determine recipient role
              let recipientRole;
              if (updatedStatus === 'ApprovedByHOD') {
                recipientRole = 'AssociateChair';
              } else if (updatedStatus === 'Rejected' || updatedStatus === 'Review') {
                recipientRole = 'Convener';
              }
              ...
              const updatedProposal = {
                ...
                rejectionMessage: updatedStatus === 'Rejected' ? message : proposal.rejectionMessage,
                reviewMessage: updatedStatus === 'Review' ? message : proposal.reviewMessage,
                clarificationMessage: updatedStatus === 'ApprovedByHOD' && proposal.status === 'AwaitingHODClarification' ? message : proposal.clarificationMessage,
              };
                ...
                prevProposals.map(p =>
                  p.id === proposal.id
                    ? {
                        ...p,
                        status: updatedStatus,
                        rejectionMessage: updatedStatus === 'Rejected' ? message : p.rejectionMessage,
                        reviewMessage: updatedStatus === 'Review' ? message : p.reviewMessage,
                        clarificationMessage: updatedStatus === 'ApprovedByHOD' && p.status === 'AwaitingHODClarification' ? message : p.clarificationMessage,
                      }
                    : p
                )
              );
              setSelectedProposal(null); // Close the popup
              alert(`Proposal ${updatedStatus.toLowerCase()} successfully!`);
            } catch (error) {
