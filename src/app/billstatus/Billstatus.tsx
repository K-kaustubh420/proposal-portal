...
// Inside ViewBills.tsx

const fetchBills = useCallback(async (userEmail: string) => {
    setLoading(true);
    try {
        const proposalsCollection = collection(db, 'eventProposals');
        const q = query(proposalsCollection, where("convenerEmail", "==", userEmail), where("status", "==", "Approved"));
...
useEffect(() => {
    ...
    if (currentUserEmail) {
        unsubscribe = fetchBills(currentUserEmail) as unknown as () => void; // CORRECT type assertion
    } else {
        setBills([]);
        setLoading(false);
    }
    ...
    const toggleStarred = async (id: string) => {
      ...
      const toggleReviewLater = async (id: string) => {
        const bill = bills.find((b) => b.id === id);
        if (bill) {
          try {
            const billRef = doc(db, "eventProposals", id);
    ...
    const handleBillClick = (bill: Proposal) => {
                  ...
                  {bills.map((bill) => (
                    <tr
                      ...
                      className={`hover:bg-gray-50 cursor-pointer ${
                        bill.read ? "bg-gray-100" : "bg-white"
